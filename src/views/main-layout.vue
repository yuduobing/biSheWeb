<template>
  <div class="container">
    ...
    <div class="top-header">
      <div class="title">
        <h1>{{ title }}</h1>
        <div class="top-header-tip" v-show="this.$store.getters.roles!=='admin'">
          <button style="display:block;" @click="logout" >退出</button>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    queryProvinceDataList () {
      let self = this
      listCovid(this.queryParams).then(res => {
   ...
   async setProvinceRankingData (areaList) {
      let provinceList = []
      let dataValueList = []

  await areaList.sort(this.arraySort('confirmedCount'))
      for (let i = 0; i <10; i++) {
      ...
      var cloneObj =[]
      cloneObj = JSON.parse(JSON.stringify(areaList));
     await   cloneObj.sort(this.arraySort('deadCount'))
      let provinceListdead = []
      let dataValueListdead= []
      for (let i = 0; i <3; i++) {
        provinceListdead.push(cloneObj[i].provinceLabel)
        dataValueListdead.push(cloneObj[i].deadCount)

      }
      let data2 = {
        provinceList: provinceListdead,
        valueList: dataValueListdead
      }


      this.top10ProvinceData = data
      //死亡人数排行
      this.top10ProvinDead = data2
    },
